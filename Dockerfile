# using VirtualBox version 4.3.14
...
RUN mkdir -p /vboxguest && \
    cd /vboxguest && \
    curl -L -o vboxguest.iso http://download.virtualbox.org/virtualbox/4.3.14/VBoxGuestAdditions_4.3.14.iso && \
    ...
    cd amd64/src/vboxguest-4.3.14 && KERN_DIR=/linux-kernel/ make && cd ../../.. && \
    cp amd64/src/vboxguest-4.3.14/*.ko $ROOTFS/lib/modules/$KERNEL_VERSION-tinycore64 && \
    mkdir -p $ROOTFS/sbin && cp x86/lib/VBoxGuestAdditions/mount.vboxsf $ROOTFS/sbin/
 
RUN echo "" >> $ROOTFS/etc/motd; \
    echo "  boot2docker with VirtualBox guest additions version 4.3.14" >> $ROOTFS/etc/motd; \
...
RUN echo '#!/bin/sh' >> $ROOTFS/etc/rc.d/vbox-guest-additions-permanent-mount; \
    echo 'sudo modprobe vboxsf && sudo mkdir /dev-home && sudo mount -t vboxsf dev-home /dev-home -o uid=9999,gid=9999,rw' >> $ROOTFS/etc/rc.d/vbox-guest-additions-permanent-mount
